...
export function VisualNavigation(){
 ...
 const localRows=datasets=>navMethods.filter(([key])=>datasets.every(dataset=>navigation.some(r=>r.model===key&&r.dataset===dataset))).map(([key,label])=>[label,...datasets.map(dataset=>{const r=navigation.find(r=>r.model===key&&r.dataset===dataset);return number(r.sr_pct,0)+' / '+number(r.spl_pct,2);})]);
 return <Section id="visual-navigation" index={c('VISUAL NAVIGATION','视觉导航')} title={c('Visual Navigation','视觉导航')} subtitle={c('VLN-CE · R2R / RxR | ObjectNav · MP3D / HM3D v2','VLN-CE · R2R / RxR | ObjectNav · MP3D / HM3D v2')}>
  <Setup methods={[
   ...
   ['SpanNav / NavFoM',c('SpanNav is evaluated on R2R and RxR; NavFoM is evaluated on all four datasets, with 50 episodes per system and dataset.','SpanNav 评测 R2R 与 RxR，NavFoM 评测全部四个数据集；每个系统在每个对应数据集上评测 50 条轨迹。')]
  ]}>
   <p>{c('R2R and RxR evaluate following language instructions through indoor environments.','R2R 与 RxR 评测在室内按语言指令行走。')}<Cite ids={['vlnce','r2r','rxr']}/>{' '}{c('MP3D and HM3D v2 evaluate searching for an object category.','MP3D 与 HM3D v2 评测寻找指定类别的物体。')}<Cite ids={['mp3d','hm3d','hm3dsem','objectnav']}/>{' '}{c('Each system is evaluated on 50 fixed episodes per dataset. Astra, LightNav-0, Uni-NaVid, OmniNav Flow and NavFoM cover all four datasets; SpanNav covers R2R and RxR, giving 1,100 trajectories in total. R2R and RxR supply route instructions, with English guides for RxR. MP3D and HM3D v2 use closed-vocabulary object categories. Each episode permits at most 500 primitive actions. Success requires an explicit STOP within 3 m of the VLN goal or 1 m of an annotated ObjectNav goal viewpoint.','每个系统在各数据集上使用 50 个固定 episode。Astra、LightNav-0、Uni-NaVid、OmniNav Flow 与 NavFoM 覆盖四个数据集，SpanNav 覆盖 R2R 与 RxR，共计 1,100 条轨迹。R2R 与 RxR 提供路线指令，RxR 使用英文 guide 指令；MP3D 与 HM3D v2 使用闭词表物体类别。每次运行最多执行 500 个基础动作，需在 VLN 目标 3 m 内或标注的 ObjectNav 目标视点 1 m 内显式发出 STOP。')}</p>
   ...
   <ImageFigure src={mobileRoot+'assets/figures/navigation-open-models.png'} alt={c('Navigation observations, decision systems and execution adapters','导航观测、决策系统与执行适配器')} caption={c('Local execution interfaces of Astra, LightNav-0, Uni-NaVid and OmniNav Flow. OmniNav Flow receives three RGB views; Astra, LightNav-0 and Uni-NaVid receive one. The simulator executes each selected action before returning the next observation.','Astra、LightNav-0、Uni-NaVid 与 OmniNav Flow 的本地执行接口示意。OmniNav Flow 使用三个 RGB 视角，Astra、LightNav-0 与 Uni-NaVid 使用一个。仿真器执行所选动作后返回下一次观测。')}/>
  </Setup>
  <Figure title={c('Navigation success and path efficiency','导航成功率与路径效率')} subtitle={c('Local evaluation and published benchmarks','本地评测与已发表基准结果')} caption={<>{c('All values are percentages. Local evaluation uses 50 fixed episodes per system and dataset. SpanNav covers R2R and RxR; the other five systems cover all four datasets. Published rows use the validation splits, observations and execution protocols of the cited papers; their observation settings are shown beside the method names. HM3D results use the v2 closed-vocabulary setting.','数值均为百分比。本地评测中，每个系统在各数据集上评测 50 个固定 episode；SpanNav 覆盖 R2R 与 RxR，其余五种系统覆盖四个数据集。论文结果采用各自的验证集、观测与执行协议，观测配置列在方法名旁。HM3D 使用 v2 闭词表设置。')}<span className="paper-links">{Object.entries(paperNames).map(([key,label])=><span key={key}><a href={published.sources[key].url} target="_blank" rel="noreferrer">{label} ↗</a><Cite ids={key}/></span>)}</span></>}>
   ...
   <div className="charts-2 nav-results">{Object.entries(datasetNames).map(([dataset,label])=>{
    ...
    return <div key={dataset}><div className="chart-title"><h4>{label}</h4><span>{dataset==='r2r'||dataset==='rxr'?'VLN-CE':'ObjectNav'}</span></div>
     <h5>{c('Local evaluation · 50 episodes','本地评测 · 50 个 episode')}</h5><Bars decimals={metric==='sr_pct'?0:2} rows={navMethods.flatMap(([key,label,color])=>{const r=navigation.find(r=>r.model===key&&r.dataset===dataset);return r?[{label,value:r[metric],color}]:[];})}/>
  ...
  <DataTable caption={c('Local VLN-CE results · SR / SPL (%)','本地 VLN-CE 结果 · SR / SPL（%）')} headers={[c('System','系统'),'R2R','RxR']} rows={localRows(['r2r','rxr'])}/>
  <DataTable caption={c('Local ObjectNav results · SR / SPL (%)','本地 ObjectNav 结果 · SR / SPL（%）')} headers={[c('System','系统'),'MP3D','HM3D v2']} rows={localRows(['mp3d','hm3d'])}/>
  ...
  <Conclusion paragraphs={c([
   ...
   '作为 S2 导航器，Astra 在四个本地子集上的 SR 和 SPL 均高于各子集内所测的导航策略。它能基于单个 RGB 视角跨房间执行较长指令，并搜索指定类别物体；RxR 的成功率达到 92%，是本次路线跟随评测中最突出的表现。这一结果体现的是高层导航决策能力，不等同于底层运动控制能力。',
